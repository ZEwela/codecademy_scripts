  ...
  static generateField(rows, columns) {
    ...
    const chunk = (arr, rows) => {
      ...
      for (let i = rows; i > 0; i--) {
        ...
        if (xPosition === null) {
          let xPositionCheck = spliced.findIndex(
            (element) => element === pathCharacter
          );
          if (xPositionCheck !== -1) {
...
const newDirection = (direction) => {
  ...
  let position = myField.currentPosition();
  if (
    position[0] === Field.columns ||
    position[1] === Field.rows ||
    position[0] === -1 ||
    position[1] === -1
  ) {
...
process.stdout.write(
  "Get to the hat (^), you start from (*). Be careful there are holes (O) of doom.\nNow, choose wisely:\nS = south,\nN = north,\nW = west,\nE = east\nWhich way? \n"
);
